...
def scrape_2026_car_culture_chase_fixed():
    base_api_url = "https://hotwheels.fandom.com/api.php"
    page_title = "2026_Car_Culture"
    all_data = []
    ...
    print(f">>> {page_title} sayfası taranıyor (Chase araçlar feature olarak işaretleniyor)...")
    ...
    try:
        ...
        for table in tables:
            ...
            for row in rows[1:]:
                ...
                # Series Numarasını Al
                s_num = get_val("series_num")
                
                # CHASE KONTROLÜ: Eğer numara 0/5 ise feature 'chase' olur
                feature_val = ""
                if s_num == "0/5":
                    feature_val = "chase"
                ...
                all_data.append({
                    ...
                    "col_num": get_val("col_num"),
                    "image_url": img,
                    "feature": feature_val, # 0/5 olanlarda 'chase' yazar, diğerleri boş kalır
                    "scale": "1:64",
                    "category": "premium"
                })
        ...
        if all_data:
            ...
            # Çıktı sütunlarını düzenle
            df = df[["year", "series", "series_num", "model_name", "color", "toy_num", "col_num", "image_url", "feature", "scale", "category"]]
            
            output_file = "hotwheels_2026_car_culture_with_chase.csv"
            df.to_csv(output_file, index=False, encoding="utf-8-sig")
            print(f"\n✅ İşlem Tamam! {len(df[df['feature'] == 'chase'])} adet Chase araç işaretlendi.")
...
if __name__ == "__main__":
    scrape_2026_car_culture_chase_fixed()